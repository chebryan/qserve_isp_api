defmodule QserveIspApiWeb.VerifyPaymentLive do
  ...
  def mount(%{"username" => username, "mac" => mac, "payment_id" => payment_id}, _session, socket) do
    ...
    if payment_id do
      # Schedule periodic checks for payment status
  ...
  def handle_info(:check_payment, socket) do
    case Payments.get_payment_status(socket.assigns.payment_id) do
      ...
      "completed" ->
        {:noreply, redirect(socket, external: "http://login.hs/login")}
